...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <header class="site-header">
    ...
    <div class="header-note"><span class="little-spark">✦</span> Grandes sonhos começam com um impulso.</div>
    <span class="version"><span class="status-dot"></span> Protótipo <span class="version-number">v0.1</span></span>
  </header>

  <main>
    <section class="play-section" aria-labelledby="level-heading">
      <div class="section-heading">
        <div><div class="eyebrow">DUAS IRMÃS. MIL POSSIBILIDADES.</div><h1 id="level-heading">Seu primeiro <span>rolê.</span>${pixelStar}</h1></div>
        <p>Sem pressa, sem pontuação.<br>Só você, suas rodas e um pequeno sonho.</p>
      </div>

      <div class="character-picker" role="group" aria-label="Escolher personagem">
        <span class="picker-label">Quem vai pro rolê?</span>
        <div class="character-options">
          <button class="character-option" type="button" data-character="nana" aria-label="Selecionar Nana, skate" aria-pressed="true" disabled>
            <canvas id="selector-portrait-nana" width="64" height="64" aria-hidden="true"></canvas>
            <span><strong>Nana</strong><small>Skate</small></span><span class="selection-check" aria-hidden="true">✓</span>
          </button>
          <button class="character-option" type="button" data-character="nunu" aria-label="Selecionar Nunu, patins" aria-pressed="false" disabled>
            <canvas id="selector-portrait-nunu" width="64" height="64" aria-hidden="true"></canvas>
            <span><strong>Nunu</strong><small>Patins</small></span><span class="selection-check" aria-hidden="true">✓</span>
          </button>
        </div>
      ...
      <div class="game-shell" id="game-shell">
        <div class="game-topbar">
          <div class="level-name"><span class="level-flower">✿</span><strong>Jardim dos sonhos</strong><span class="level-divider"></span><span class="level-mode">Pista de teste</span></div>
        ...
        <div class="canvas-wrap">
          ...
          <div class="practice-tag"><span class="status-dot"></span> TREINO LIVRE</div>
          <div class="start-hint" id="start-hint"><span class="hint-keys">← <span>→</span></span><span>Um impulso e a gente começa.</span></div>
      ...
      <div class="controls-strip" id="controls-description">
        ...
        <div class="control-guide"><kbd class="space-key">espaço</kbd><span>Pular <small>segure para ir mais alto</small></span></div>
      ...
      <div class="below-game">
        <div class="friendly-tip"><span class="tip-spark">${pixelStar}</span><p><strong>Encontre o seu ritmo.</strong> Solte a direção e sinta as rodas deslizarem.</p></div>
...
const game = new Game(canvas, element('touch-controls'), {
  onFrame(snapshot) {
    element('movement-label').textContent = stateLabels[snapshot.state];
    element('current-state').textContent = snapshot.state;
    element('speed-value').textContent = String(Math.round(Math.abs(snapshot.vx)));
    element('speed-fill').style.transform = `scaleX(${Math.min(1, Math.abs(snapshot.vx) / game.config.maxSpeed)})`;
  ...
  onAudio(ready, error) {
    ...
    element('audio-status').textContent = error ?? (game.audio.muted ? 'Som desativado' : 'Tocando: Sonhinho sobre rodas');
  },
...
function updateCharacterSelection(id: CharacterId) {
  ...
  canvas.setAttribute('aria-label', `Pista da ${character.label}, de ${character.gear}. Use setas ou analógico para andar. Espaço ou A para pular. P ou Start para pausar. 1/2 ou LB/RB para trocar personagem.`);
  element('announcement').textContent = `${character.label} selecionada, de ${character.gear}.`;
}
...
async function prepareArt() {
  ...
  try {
    ...
    overlay.hidden = true;
    element('announcement').textContent = 'Nunu e Nana estão prontas. Escolha sua personagem e comece o rolê.';
  } catch (error) {
...
function toggleSound() {
  ...
  element('audio-status').textContent = game.audio.muted ? 'Som desativado' : soundReady ? 'Tocando: Sonhinho sobre rodas' : 'O som começa quando você jogar';
  canvas.focus({ preventScroll: true });
}
